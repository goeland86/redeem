...
class Safety(Unit):
  ...
  def __init__(self, name, options, printer):
    ...
    self.min_temp = float(self.options[
        "min_temp"])    # If temperature falls below this point from the target, disable.
    self.max_temp = float(
        self.options["max_temp"])    # Max temp that can be reached before disabling printer.
    self.max_rise_rate = float(
        self.options["max_rise_rate"])    # Fastest temp can rise pr measrement
    self.max_fall_rate = float(
        self.options["max_fall_rate"])    # Fastest temp can fall pr measurement
    self.min_rise_rate = float(
        self.options["min_rise_rate"]
    )    # Slowest temp can rise pr measurement, to catch incorrect attachment of thermistor
    self.min_rise_offset = float(
        self.options["min_rise_offset"]
    )    # Allow checking for slow temp rise when temp is below this offset from target temp
    self.min_rise_delay = float(
        self.options["min_rise_delay"]
    )    # Allow checking for slow temp rise after this delay time to allow for heat soak
  ...
  def initialise(self):
    ...
    disconnect = False
    if (not isinstance(input_sensor, TemperatureSensor)) and (not isinstance(input_sensor,
                                                                             ColdEnd)):
  ...
  def run_safety_checks(self):
    ...
    if temp_rate > self.max_rise_rate:
      a = Alarm(
          ...
          "Temperature rising too quickly ({:.2f} degrees/sec) for {} ({} = {:.2f})".format(
              temp_rate, self.name, self.input.name, current_temp))

    # Check that temperature is not rising quickly enough when power is applied
    check = [
        temp_rate < self.min_rise_rate, power_on,
        current_temp < (target_temp - self.min_rise_offset), heating_time > self.min_rise_delay
    ]
    if np.all(check):
      a = Alarm(
          Alarm.HEATER_RISING_SLOW,
          "Temperature rising too slowly ({:.2f} degrees/sec) for {} ({} = {:.2f})".format(
              temp_rate, self.name, self.input.name, current_temp))
    # Check that temperature is not falling too quickly
    if temp_rate < -self.max_fall_rate:
      a = Alarm(
          Alarm.HEATER_FALLING_FAST,
          "Temperature falling too quickly ({:.2f} degrees/sec) for {} ({} = {:.2f})".format(
              temp_rate, self.name, self.input.name, current_temp))
    ...
    if self.min_temp_enabled and (current_temp < (target_temp - self.min_temp)):
      a = Alarm(
          ...
          "Temperature of {:.2f} below min set point ({:.2f} degrees) for {}".format(
              current_temp, self.min_temp, self.name))
    # Check if the temperature has gone beyond the max value
    if current_temp > self.max_temp:
      a = Alarm(
          Alarm.HEATER_TOO_HOT, "Temperature of {:.2f} beyond max ({:.2f} degrees) for {}".format(
              current_temp, self.max_temp, self.name))
    # Check the time diff, only warn if something is off.
    if time_delta > 4:
      logging.warning("Time between updates too large: " + self.name + " temp: " +
                      str(current_temp) + " time delta: " + str(time_delta))
  ...
  def get_options(self):
    ...
    for command in self.command:
      if command in self.printer.command_connect:
        logging.warning("multiple instances of {} used in [Temperature Control]".format(
            self.command))
